#!/usr/bin/env bash
set -euo pipefail
D="$(dirname "$0")/../.scion/templates/planner-t4"
...
grep -q "max_turns: 100" "${D}/scion-agent.yaml" || { echo "FAIL: wrong max_turns"; exit 1; }
grep -q 'max_duration: "4h"' "${D}/scion-agent.yaml" || { echo "FAIL: wrong max_duration"; exit 1; }
grep -q "spec-kit" "${D}/scion-agent.yaml" || { echo "FAIL: missing spec-kit skill"; exit 1; }
echo "PASS"
