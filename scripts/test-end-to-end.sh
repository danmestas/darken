...
#
# Behavior: copies the repo into a tmp dir, runs `make darken` to
...
cd "${TMP}/factory"
make darken
bin/darken bootstrap
bin/darken doctor
bin/darken spawn smoke-r --type researcher "echo hi"
bin/darken spawn smoke-s --type sme --backend codex "what is 1+1?"
echo "PASS"
