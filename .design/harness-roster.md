...
| `darwin` | codex | gpt-5.5 | 50 | 4h | false | — | Post-pipeline evolution agent; reads completed sessions and emits YAML recommendations for `darkish apply` |
...
`darwin` never mutates harness state directly — it writes recommendations only. Application is gated by `darkish apply`, which presents each recommendation to the operator (`y/n/skip/edit`) before mutating manifests, committing changes, or re-staging skills (spec §12.4).
